...
- 拆分共享开关：沿用 `global_memory` 控制按人跨会话共享，新增 `share_group_across_groups`（默认关闭）仅控制同平台实例的 group 跨群共用。
- 归一 AstrBot 会话类型；统一主召回、关联补充与列表的读取权限，修复他人 private 记忆经关联放行的问题，保留多主体成员共享。
- 提取与写入校验可信主体，不从昵称或正文 `User ID:` 猜归属；无效 UMO 不提交提取进度，不自动改写历史记忆。
- 新增查询字符和时间预算；当前提问优先，长度超限全轮最多缩短重试一次，dense 失败保留本地检索与其他作用域结果。

## v0.5.1 (2026-09-03)

### 新增
- **夜间提取 ConversationV2 兜底源**：平台消息历史无数据的会话（私聊、未开启群消息历史开关的群）改用 LLM 对话检查点提取——按消息数切段（`maintenance_extract_conv2_chunk_messages`，默认 40），闭环判定与挂起机制与主路径一致；游标改用内容锚点续跑，锚点丢失（上下文窗口滑动/历史重置）时保守回退只处理最新一段并重锚，防止重复提取。开关 `maintenance_extract_conv2_enabled`（默认开启）。
...
### 修复
- **空记忆池召回空转**：知识库无活跃记忆时，自动注入与记忆召回直接短路返回，不再产生查询优化 LLM 调用与各召回通道的 embedding 请求。
- **小候选集无效 rerank**：融合后候选数不超过最终召回数量时跳过重排序模型调用；该场景下 rerank 无法改变入选集合，且其排序会被信号重排覆盖。

### 变更
- **矛盾检测闭环**：审核员批准的矛盾操作不再静默落空，转入待审队列由管理员裁决；管理员通过 `/memory review approve` 批准后才真正废弃被取代的旧记忆。
